...
function loadState(){
  try{
    ...
    if (!j){
      // First-time load: start with the first preset selected so new users have a named starting point
      activePresetId = HINA.PRESETS[0].id; params = HINA.presetParams(HINA.PRESETS[0]); return;
    }
...
function buildExprBar(){
  ...
  // Roving tabindex: arrow keys navigate within the toolbar (ARIA toolbar pattern)
  bar.addEventListener('keydown', e=>{
    const btns=[...bar.querySelectorAll('.eBtn')];
...
function onParam(k){
  ...
  if (k==='outfit' || k==='springOff') renderBody(false);
}
...
function renderOut(bd){
  ...
  }
  {
    const pstj=el('button',{class:'btn wide', onclick:()=>{
      const _pfail=()=>{ pstj.textContent='!'; setTimeout(()=>{ pstj.textContent=t('btn.pasteJson'); },1500);
        const sr=$('srStatus'); if(sr) sr.textContent=t('btn.pasteJson.err'); };
...
// Restore focus to the trigger button when dialog closes (keyboard/AT users lose position otherwise)
$('aboutDlg').addEventListener('close',()=>{ $('btnAbout').focus(); });
