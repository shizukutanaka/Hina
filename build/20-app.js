...
(function _probeLS(){
  try{
    const K='hina.probe', V=String(Date.now());
    localStorage.setItem(K, V);
    if (localStorage.getItem(K) !== V) _lsBroken = true;
    localStorage.removeItem(K);
  }catch(_){ _lsBroken = true; }
})();
function loadState(){
  try{
...
function saveState(){
  ...
  _saveTimer = setTimeout(()=>{
    try{
      ...
      if (!wasBroken) showErr(t('hint.saveFail'));
    }
  }, 500);
}
...
function poseAndSkin(time){
  ...
  const gq = reduceMotion ? M.qid() : M.qMul(M.qAxis([0,1,0], gazeX*0.22), M.qAxis([1,0,0], -gazeY*0.14));
...
/* ---------- camera input ---------- */
(()=>{
  // Round 528: the rotate anchor is per-pointer (ptrs holds each pointer's own last position),
  // not a shared px/py. The shared anchor was the §3-4 view-jump bug: a second finger's
  // pointerdown overwrote it, so when that finger lifted, the first finger's next move computed
  // its delta against the OTHER finger's position — measured 1.5 rad of spin for a 10px move
  // (CDP touch repro). Deleting the shared state deletes the defect: the Map already stores
  // exactly the right anchor for every pointer, in both the drag and pinch branches.
  let drag=false;
  const ptrs=new Map();
  cv.addEventListener('pointerdown',e=>{
    cv.setPointerCapture(e.pointerId); ptrs.set(e.pointerId,[e.clientX,e.clientY]);
    drag=true; cv.classList.add('drag');
  });
  cv.addEventListener('pointermove',e=>{
    ...
    // a pointer that never went down on the canvas (e.g. the mouse hovering across it while a
    // touch drag is in progress) must not steer the camera or join the pinch math
    ...
    camYaw -= (e.clientX-prev[0])*0.008;
    camPitch = M.clamp(camPitch+(e.clientY-prev[1])*0.006, -0.5, 1.25);
    ptrs.set(e.pointerId,[e.clientX,e.clientY]);
  });
...
function paramRow(k){
  ...
  if (s.k==='num'){
    ...
    const r=el('input',{id:pid, type:'range',min:s.min,max:s.max,step:s.step,value:params[k],
      ...
      ondblclick:()=>{ captureUndo(); params[k]=s.def; r.value=String(s.def);
        r.setAttribute('aria-valuetext',fmtNum(s.def,s.step));
        if (valEl) valEl.tagName==='INPUT' ? valEl.value=String(s.def) : (valEl.textContent=fmtNum(s.def,s.step));
        onParam(k);
      ...
      oninput:e=>{ params[k]=parseFloat(e.target.value);
        r.setAttribute('aria-valuetext',fmtNum(params[k],s.step));
        if (valEl.tagName==='INPUT') valEl.value=String(params[k]); else valEl.textContent=fmtNum(params[k],s.step);
        onParam(k); }});
    if (mode==='detail'){
      valEl=el('input',{type:'number',class:'num numIn',min:s.min,max:s.max,step:s.step,value:params[k],
        ...
        onchange:e=>{ captureUndo(); let n=parseFloat(e.target.value);
          const _announce=(v)=>{ e.target.setAttribute('aria-invalid','true'); e.target.setAttribute('aria-errormessage','srAlert');
            showErr(t('a11y.clamped').replace('{v}',v));
          ...
          params[k]=clamped; e.target.value=String(clamped); r.value=String(clamped); r.setAttribute('aria-valuetext',fmtNum(clamped,s.step)); onParam(k); }});
      ...
      valEl=el('span',{class:'num'}, fmtNum(params[k],s.step));
    }
  ...
  const inp=el('input',{id:pid, type:'color', value:params[k], 'aria-label':label,
    ...
    oninput:e=>{params[k]=e.target.value; onParam(k); updateSwPressedState();
      const ni=swBtns.findIndex(b=>b.dataset.c===e.target.value); if(ni>=0) setSwTab(ni);},
    onchange:e=>{ const sr=$('srStatus'); if(sr) sr.textContent=label+': '+((HINA.PAL_NAMES[e.target.value]||{})[lang]||e.target.value); }});
...
function renderExprEditor(bd){
  ...
  for (const name of HINA.EXPR_EDITABLE){
    ...
    for (const morphKey of HINA.EXPR_INGREDIENTS){
      ...
      // full setExpr() path when actually switching which expression is being previewed; otherwise
      // just resync morphW directly from the updated mix.
      const applyValue = n => {
        if (n>0) exprMix[name][morphKey]=n; else delete exprMix[name][morphKey];
        if (activeExpr!==name) setExpr(name);
        else { const mx=exprMix[name]; morphW={}; for(const m in mx) morphW[m]=mx[m]/100; morphDirty=true; }
        saveState();
      };
      ...
      numEl = el('input',{type:'number', class:'num numIn', min:'0', max:'100', step:'1', value:v,
        ...
        onchange:e=>{ captureUndo(); let n=Math.round(Number(e.target.value));
          ...
          const _announce=(cv)=>{ e.target.setAttribute('aria-invalid','true'); e.target.setAttribute('aria-errormessage','srAlert');
            showErr(t('a11y.clamped').replace('{v}',cv));
            setTimeout(()=>{ e.target.removeAttribute('aria-invalid'); e.target.removeAttribute('aria-errormessage'); },1500); };
          if (!Number.isFinite(n)){ const cur=exprMix[name][morphKey]||0; _announce(cur); n=cur; }
...
function renderBody(scrollReset=true){
  ...
  if (activeTab==='preset'){
    ...
    const seedIn=el('input',{type:'number',class:'num numIn',style:'flex:1;min-width:0',
      ...
      onwheel:e=>e.preventDefault(),
      // Round 546: Enter must NOT fall through to its default action. blur() below fires change,
      // which runs runGacha(typed seed) — and runGacha ends by focusing the gacha button so the
      // roll can be repeated. The same Enter keystroke then activated that freshly focused button,
      // instantly rolling a RANDOM seed over the one the user just typed. Measured 3/3: typing
      // 987654 and pressing Enter left the field showing 3872386978 / 1944695725 / 3229565579 and
      // an avatar that was not seed 987654's, silently. Tab (blur without a default action) was
      // always correct, which is what made this invisible. The field carries enterkeyhint='go', so
      // Enter is the gesture it invites — on a phone keyboard it is literally labelled "go".
    ...
    const cpBtn=el('button',{type:'button', class:'btn',style:'padding:4px 8px;font-size:11px;flex:none',
      ...
      onclick:()=>{
        ...
        if (!navigator.clipboard?.writeText){ cpBtn.textContent='!'; setTimeout(()=>{ cpBtn.textContent=t('btn.copySeed'); },1500); showErr(t('btn.copySeed.err')); return; }
...
function renderOut(bd){
  ...
  const txt=(key,mk,ph)=>{ const id='meta-'+mk; bd.append(el('div',{class:'row'},
    ...
    el('input',{id, type:'text', maxlength:'256', autocomplete:'off', spellcheck:'false', autocorrect:'off', autocapitalize:'none', enterkeyhint:'done', value:meta[mk]||'', placeholder:ph||'', onfocus:()=>captureUndo(), oninput:e=>{meta[mk]=e.target.value; saveState();}}))); };
  ...
  bd.append(el('div',{class:'row'},
    ...
    el('input',{id:'meta-title', type:'text', maxlength:'256', autocomplete:'off', spellcheck:'false', autocorrect:'off', autocapitalize:'none', enterkeyhint:'done', 'aria-describedby':'fnPreview', value:meta.title||'', placeholder:t('out.title.ph'),
      onfocus:()=>captureUndo(), oninput:e=>{ meta.title=e.target.value; saveState(); updateFnPrev(); }})));
  updateFnPrev();
  bd.append(fnPrev);
  ...
  selRow('out.license','license',
    ...
    'license', v=>{
      ...
      if (v==='Other'){ licUrlInp.setAttribute('aria-required','true'); const u=document.getElementById(licUrlId); if(u) u.focus(); const sr=$('srStatus'); if(sr) sr.textContent=t('out.license.url'); }
      else { licUrlInp.removeAttribute('aria-required'); if (focusWasInUrl){ const s=document.getElementById('sel-license'); if(s) s.focus(); } }});
  bd.append(licUrlRow);
...
// localized-prefix + informative-name title; applyLang() (Round 449) set a localized-but-generic
...
async function saveJson(){
  ...
  // showSaveFilePicker() resolving and w.close() resolving could launch two independent
  // serialize+write sequences targeting the same suggested filename; whichever close() resolves
  // last wins with no ordering guarantee, so a *staler* snapshot could silently overwrite a
  // newer one while the SR announcement from the later call falsely reports success.
  ...
  const _wasFocused = btn && document.activeElement===btn;
  if(btn){ btn.disabled=true; btn.setAttribute('aria-busy','true'); }
  const _done=()=>{ _savingJson=false; if(btn){ btn.disabled=false; btn.removeAttribute('aria-busy'); if(_wasFocused && btn.isConnected) btn.focus(); } };
...
function applyLang(){
  ...
  if (!$('aboutDlg').open) $('btnAbout').setAttribute('aria-expanded', 'false');
  $('btnAbout').setAttribute('aria-keyshortcuts', '?');
  $('btnMode').setAttribute('aria-keyshortcuts', 'M');
  $('aboutClose').setAttribute('aria-keyshortcuts', 'Escape');
...
document.querySelectorAll('.rankBadge').forEach(b=>{
  ...
  const goStats=()=>{ const chg=activeTab!=='out'; activeTab='out'; renderTabs(); renderBody(chg); $('tab-out').focus(); };
...
document.addEventListener('keydown',e=>{
  ...
  if (!dlgOpen){
    ...
    if ((e.key==='m'||e.key==='M') && !e.ctrlKey && !e.metaKey && !e.altKey && notField){
      ...
      const wasOnTabNav = $('tabs').contains(document.activeElement);
      mode=mode==='easy'?'detail':'easy'; saveState(); applyLang();
      if (wasInTabBody) $('btnMode').focus();
      else if (wasOnTabNav){ const ft=$('tab-'+activeTab); if(ft) ft.focus(); }
    ...
    if (notField && !e.ctrlKey && !e.metaKey && !e.altKey && /^[1-8]$/.test(e.key)){
      ...
      if (ti<TABS.length){ e.preventDefault(); const chg=TABS[ti]!==activeTab; activeTab=TABS[ti]; renderTabs(); renderBody(chg); $('tab-'+TABS[ti]).focus(); }
    }
  }
