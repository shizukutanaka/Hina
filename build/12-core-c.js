...
function exportVRM(build, p, meta, pngBytes, thumbPngBytes, exprMix){
  ...
  /* VRM meta (sanitized) */
  const str=(v,d)=>{ const s=typeof v==='string'?v.replace(/[\u0000-\u001f]/g,'').slice(0,256):''; return s.trim()?s:d; };
...
// Round 526 (Musk-method "delete" pass): SKIRT_OUTFITS and buildSkeleton were removed from the
// public surface below — both are used only INSIDE core (SKIRT_OUTFITS by hasSkirt(); buildSkeleton
// by buildAvatar()), and nothing in the app or tests referenced HINA.SKIRT_OUTFITS /
// HINA.buildSkeleton. The definitions stay; only the unnecessary export is gone, shrinking the
...
return {
  ...
  HB, buildAvatar, BinWriter, exportVRM, selfTest,
};
})();
