...
function exportVRM(build, p, meta, pngBytes, thumbPngBytes){
  ...
  /* morph targets (full delta arrays; min/max mandatory for POSITION) */
  const targetNames = build.morphs.names.slice();
  const targets = targetNames.map(name=>{
    const full=new Float32Array(nV*3);
    for(const e of build.morphs.sparse[name]){ full[e[0]*3]=e[1]; full[e[0]*3+1]=e[2]; full[e[0]*3+2]=e[3]; }
    const mm=minMax3(full);
    return {POSITION: addAcc(addView(bw.push(full),full.byteLength,34962),5126,nV,'VEC3',{min:mm.min,max:mm.max})};
  });
  ...
  const materialProperties=[{
    ...
    floatProperties:{
      ...
      _OutlineColorMode:0, _OutlineWidthMode:1, _OutlineCullMode:1,
...
function selfTest(){
  ...
  T('accessors within buffer', ()=>{
    ...
    return ex.json.accessors.every(a=>{
      ...
      return a.count*SZ[a.componentType]*N[a.type] <= v.byteLength &&
             v.byteOffset+v.byteLength <= ex.json.buffers[0].byteLength;
    });
  });
