...
この文書は、AIコーディングセッション（コミット履歴上「Round 464」〜「Round 477」と呼ぶ14回の改善サイクル）で実施した機能監査の結果を、**前提知識のない後続セッションが読んで作業を引き継げる形**で記録したものである。

- 「Round N」はコミットメッセージ先頭の通し番号。`git log --oneline` で対応コミットを特定できる
- 本文書作成時点のテスト数は **1806 passed / 0 failed**（`node tests/run.js`）
- 次に新しい変更を行う場合の通し番号は **Round 479**（Round 478 = 本文書の追加）
...
### 3-1. 表情エディタ（v0.3項目）— **次に着手するならこれを推奨**

既存の17種 blendShapeGroups（`build/12-core-c.js`）と表情プレビューバー（`build/20-app.js` の `buildExprBar`）の基盤上で、モーフの**組み合わせ・強度調整**のみなら新規メッシュ形状が不要。つまり視覚的な品質リスクなしにテキスト・テストだけで検証可能。

### 3-2. 複数段Undo/Redo

現状は単発Undo（`build/20-app.js` の `_undoSnap` が単一スナップショット。Ctrl+Z 2回目は「履歴なし」）。技術的障害はないが、スタック上限・Redo有無・メモリ方針などの設計判断が必要な中規模改修。ユーザーの明示的な指示を得てから着手すること。

### 3-3. 指ボーン＋指モーフ（v0.2項目）— **視覚検証手段の確保が前提条件**
...
### 3-4. 髪型・衣装プリセット追加（v0.2項目）

3-3と同じ理由（新規メッシュ形状の視覚品質を保証できない）で見送り。同じ前提条件で解除。

### 3-5. v1.0系（VRM 1.0出力・VRMA・プリセット共有形式）
...
| # | 却下した機能 | 判断理由 |
|---|------|---------|
| 1 | Undo/Redoフルスタックを小修正の一環として実装 | 「1ラウンド=1つの焦点を絞った修正」の原則を超える。§3-2として明示的な指示待ちに格下げ |
