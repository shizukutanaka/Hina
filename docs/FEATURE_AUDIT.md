...
### GLB/VRM仕様準拠性（4件・core層、Khronos公式バリデータ＋VRM 0.x公式スキーマ＋MToon公式定数を一次ソースとして照合）
...
| 528 | **最後の既知欠陥（§3-4 マルチタッチ視点ジャンプ）を修正**。回転ドラッグ中に2本目の指が触れて離れると視点が跳ぶ問題。**機構**: `pointerdown` が共有アンカー `px/py` を無条件に上書きするため、指Bの座標で汚染され、Bが離れた後に指Aが動くと `A現在 - B最終` がデルタとして適用される。**CDP実タッチイベントで再現**（キャンバス内に2点配置）: Aを10px動かしただけで **|ΔcamYaw| = 1.504 rad（約86°）**、理論値 `(Ax-Bx)*0.008 = -1.504` と完全一致。**修正は状態の削除**（マスク法②「最良の部品は部品なし」）: 共有 `px/py` を削除し、既に per-pointer で正しい位置を保持している `ptrs` Map から `ptrs.get(e.pointerId)` でデルタを計算。併せて `if(!ptrs.has(e.pointerId)) return;` を追加し、canvas上でdownしていないポインタ（タッチdrag中のマウスhover等）がカメラを動かす/pinch計算へ混入するのを防止。**修正後の実測**: 同一シーケンスで **0.080 rad**（10px相当の正常値）、通常50pxドラッグ -0.400 rad（期待通り）、ピンチも camDist 2.683→1.341 で正常。**変異テストで実証**: 旧共有アンカー実装に戻すと Round 528 の6テストすべてがFAIL。**再現手順**: CDP `Input.dispatchTouchEvent` で「A down→A move→B down→B up→A move」、`camYaw` を window へ露出して計測（両点をキャンバス矩形内に置くこと——外だとイベントが届かない） | `build/20-app.js` のポインタハンドラ |
...
| 522 | **アウトライン書き出しが一部のVRM消費実装で描画されない不具合**。MToonのアウトラインパスはシェーダの**コンパイル時キーワード**（`#if defined(...)`）でバリアント選択され、`_OutlineWidthMode` のfloat値**だけ**では選択されない。雛はアウトラインON時に `_OutlineWidthMode=1`（World）と `_OutlineColorMode=0`（Fixed）をfloatで設定していたが、対応する keywordMap は `_ALPHATEST_ON` のみで、`MTOON_OUTLINE_WIDTH_WORLD` と `MTOON_OUTLINE_COLOR_FIXED`（キーワード名はMToon公式 `Utils.cs` の定数 KeyOutlineWidthWorld/KeyOutlineColorFixed から取得）が**欠落**していた。キーワードでバリアントを選ぶ消費実装（three-vrm / Babylon.js / Godot の各MToon移植は `#if defined` を直接使用——WebSearchで確認）では、floatが設定されていてもアウトラインが描画されない。floatからキーワードを再導出するインポータ（UniVRM等の可能性）では冗長だが無害。**発見手法**: Round 506/514/515と同じ「一次ソースを取得し実バイナリの実値と照合」（MToon公式スコープでkeywordMap/floatの整合を検査）。**修正**: `p.outline` の時のみ keywordMap に両キーワードを追加（アウトラインOFFの既定は不変）。マテリアルJSONのみの変更でジオメトリ不変（render-check全13ケースok）。**変異テストで実証**: 修正を戻すと両キーワードのテストがFAIL。幅=Screen/色=Mixed の不使用キーワードを誤って設定しないことも検証 | `build/12-core-c.js` の materialProperties keywordMap |
...
17. **UV座標・テクスチャアトラスの整合性（Round 523で実測・クリーン）**: 実バイナリの TEXCOORD_0 accessor を抽出して検証。(1) 全UVが [0,1] 内（実測レンジ U[0,0.981] V[0,0.781]、範囲外ゼロ）、(2) TEXCOORD は min/max 未宣言（glTFはPOSITIONのみ必須なので違反なし。宣言する場合はRound 506同様の厳密一致が必要になるが、現状は宣言していないので問題なし）、(3) 顔パーツ6種（eyeL/eyeR/browL/browR/mouth）のUVバウンディングボックスがいずれも意図した `ATLAS` 矩形内に正確に収まる＝領域の取り違え・V軸反転なし、(4) 矩形領域に相互の重なりなし（目/眉/口は上段で異なるx、blushは口の下）、(5) 単色ブロック8種は各64pxブロックの**中心**でサンプリング（頂点UVが0.5texel以内に集中）＝mip縮小時の隣接領域ブリードを設計で回避。**変更不要**
18. **法線ベクトルの健全性（Round 523で実測・クリーン）**: 全パラメータ域（デフォルト+全プリセット+ガチャ60シード+全パラメータ端、計172,642法線）で、(1) 全て単位長（worst `|len-1|` = 3.3e-16 = float32 epsilon級）、(2) 零ベクトルゼロ、(3) NaN/Inf ゼロ。NORMAL accessor は count==POSITION・FLOAT VEC3。外向き性は Round 519 で頭部球面221/225により独立確認済み。**変更不要**
20. **参照Web VRMローダー three-vrm での実読込（Round 525で実施・全解決）**: 公式バリデータ（構造の静的検証）に加え、実際のローダーが受理するかを検証。`@pixiv/three-vrm`（pixiv製・最も普及したJS/WebのVRMローダー。Round 522でMToonキーワード欠落の影響を受ける消費実装として特定した当事者）を npm 導入し、GLTFLoader + VRMLoaderPlugin で雛の実VRMを headless パース。デフォルト+アウトラインON+ガチャシード+プリセット+揺れ物OFF=5ケースすべてで: **humanoid必須15ボーン全解決（欠落ゼロ）**、**expression 17個すべて解析**、**springBoneManager解決（8ジョイント／揺れ物OFF時は正しく0）**、firstPerson・meta も解決。アウトラインONも正常読込＝Round 522の修正が three-vrm のパースを壊していないことも確認。唯一の警告はテクスチャ読込失敗だが、これは node にcanvas/GLが無いための環境由来（PNGバイト自体は公式バリデータで0 errors＝正常）で**雛の欠陥ではない**。構造バリデータと参照ローダーの**両方**が雛の出力を裏付けた。21. **書き出しのエンドツーエンド検証（Round 529で実施・成功）**: これまで書き出しは **Node から `exportVRM()` を呼ぶ形でしか検証されておらず、「ユーザーが実際に押すボタン」は一度も通していなかった**。Playwright実ブラウザで Export タブ→名前入力→「Export VRM」クリックまで実行し、**`E2ETest.vrm`（166,124バイト）が実際にダウンロードされる**ことを確認。ダウンロードされたファイル自体を検証: GLBマジック `0x46546c67` 正当、ヘッダ宣言長 == 実ファイルサイズ、そして**公式 glTF-Validator で 0 errors**。つまり「ユーザーが手にするファイル」が仕様準拠であることを初めて実証した。**両保存経路を検証**: (a) File System Access API 非対応ブラウザ（Firefox/Safari等・`showSaveFilePicker` を削除して模擬）→ ダウンロード経路が正常動作、(b) 対応ブラウザ（Chrome/Edge）は保存ダイアログを開くためヘッドレスでは駆動不能だが、**ユーザーがダイアログをキャンセルした場合**（AbortError）を模擬したところ、ボタンは "Export VRM" に復帰・disabled にならず・ページエラーなし・**キャンセル後の再試行も成功**＝再入ガードが正しく解放されている。**恒久化**: `tools/render-check.js` に統合し、以後は視覚回帰と同時に書き出しE2Eも毎回検証される | `tools/render-check.js` の export end-to-end ブロック |

22. **自己診断と自動保存のE2E検証（Round 530で実施・両方クリーン）**: Round 529 で「書き出しボタンは一度もE2E検証されていなかった」穴が見つかったため、同じ問いを残る2つのユーザー向け約束にも適用した。(1) **`?selftest`**——「単一HTML＝監査容易」という中核価値の実装。実ブラウザで `index.html?selftest` を開き **「Self-test: all OK — 22/22 checks」** を確認、`HINA.selfTest()` も `ok:true`、ページエラーゼロ。(2) **自動保存の往復**——実UIで髪型を変更＋出力タブで名前入力 → **リロード** → UIのselect値・名前欄・localStorage・`document.title` すべて復元を確認（`Hina — PersistTest`）。**恒久化**: 両方を `tools/render-check.js` に追加し、以後は視覚回帰・書き出しE2Eと合わせて毎回検証される。**変異テストで実証**: selfTestの1チェックを強制失敗させる→FAIL、localStorage書込みを無効化→FAIL。**副産物の設計知見**: 保存経路は `saveState()` のデバウンス保存と `_emergencySave()`（ページ離脱時）の**2系統あり冗長**——片方を無効化しただけでは往復が成功してしまい、両方潰して初めてFAILした。作業消失に対する二重の防御になっている | `tools/render-check.js` の self-test / persistence ブロック |

23. **共有URLとUndo/RedoのE2E検証（Round 531で実施・両方クリーン・E2E網羅完結）**: (1) **`?seed=N` 共有URL**——「リンクを送れば同じアバターが出る」という配布上の約束。決定論だけの検査は「シードを無視して常に同じ結果を返す」実装でも通ってしまうため、**同一シード再現（seed=12345を別コンテキストで2回→パラメータ署名一致）と別シード相違（seed=999→異なる）の対**で検証し両方成立。(2) **Undo/Redo**——実キーボードイベントで `Ctrl+Z`/`Ctrl+Shift+Z` を送出し、髪型 twin→short→undo(twin)→redo(short) の往復を確認。ページエラーなし。**恒久化**: 両方を `tools/render-check.js` に追加（描画13ケース＋export＋selftest＋persistence＋seed＋undo/redoの計6系統のE2Eが1コマンドで走る）。**変異テストで実証**: seed URL解析を無効化→「different seed produced the same avatar」でFAIL、`doUndo()` 呼出を無効化→「Ctrl+Z did not restore」でFAIL。これで**主要なユーザー向け約束すべてが実ブラウザでE2E検証・恒久化**された | `tools/render-check.js` の share URL / undo-redo ブロック |

**後続セッションへ**: `npm install three @pixiv/three-vrm` 後、GLTFLoaderにVRMLoaderPluginを登録して `loader.parse(arrayBuffer,...)` で読込確認できる。**Round 527で追加検証**: (1) 表情プリセットの**意味的マッピング**——three-vrm が雛のVRM0 blendShapeGroup を VRM標準プリセット名へ正しく変換するか。`em.getExpression(name)` で17プリセット全て解決（`joy→happy`・`fun→relaxed`・`sorrow→sad`・`a/i/u/e/o→aa/ih/ou/ee/oh`・`blink_l/r→blinkLeft/Right`、欠落ゼロ）＝VRChatや任意のthree-vrm消費者が正しい表情を発火できる。(2) **metaの往復**——title/author/allowedUser(`ExplicitlyLicensedPerson`)/commercial/license(`CC_BY_NC_SA`)が three-vrm で正しく読み戻る＝Round 526で削除しかけた `license.*`/`allowed.*` キー群が実ローダーで確かに機能していることの二重確認

19. **公式 Khronos glTF-Validator による検証（Round 524で実施・0 errors）**: これまでの仕様準拠検査は `ISSUES.md` を転記した手書きチェックだったが、本ラウンドで**公式バリデータ本体**（npm `gltf-validator`、環境のプロキシ経由で一時ディレクトリに導入可能。`validator.validateBytes(u8)`）を雛の実バイナリに対して実行。デフォルト+アウトラインON+揺れ物OFF+ガチャ2シード+全6プリセット=**11書き出しすべてで 0 errors**。手書きチェック（Round 515スイート）の正しさを権威ある実装が裏付けた。出た警告は2種のみで、**いずれもVRM 0.xの正しい構造に内在する想定内**（実害なし・修正不可）: (a) `INVALID_EXTENSION_NAME_FORMAT`（拡張名 "VRM"）——VRM 0.x仕様が拡張名を literally "VRM" と規定（glTFのベンダー接頭辞規約 `Prefix_Name` に反するが、VRM1の `VRMC_vrm` とは異なりVRM0では不可避。全VRM0ファイルで出る）、(b) `NODE_SKINNED_MESH_NON_ROOT`（スキンメッシュノード Body がシーンroot直下でなくRoot配下）——VRM0はメッシュを単一Rootノード配下に置く慣例（UniVRMも同構造）で、Body の唯一の祖先 Root は**単位変換**のため「親変換が無視される」警告は実質無意味。構造をいじって警告を消すのは UniVRM 取込との互換性を崩すリスクがあり、本環境では実機検証不能なため**変更しない**。**後続セッションへ**: `cd <tmpdir> && npm install gltf-validator` で公式バリデータを導入し実バイナリを検証できる（オフライン不可の環境ではSKIP）
